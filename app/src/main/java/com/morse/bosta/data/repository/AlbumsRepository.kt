...
class AlbumsRepository @Inject constructor(private val api: Api, private val cache: Cache) :
    IAlbumsRepository {
    ...
    override fun loadAlbumPhotos(albumId: Int): Flow<List<PhotosResponseItem>> {
        return execute(!cache.isAlbumExist(albumId), {
            val networkResponse = api.getAlbumPhotos(albumId)
            cache.saveAlbumPhotos(albumId, networkResponse)
        }, {
            cache.loadAlbumPhotos(albumId)
        })
    }
}